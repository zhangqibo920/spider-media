          ...
          <el-table :data="tasks" v-loading="loadingTasks">
            ...
            <el-table-column label="操作" width="150">
              <template #default="{ row }">
                ...
                <el-button v-if="row.status === 0" type="primary" link @click="handleSchedule(row.id)">定时</el-button>
              </template>
            </el-table-column>
          </el-table>
        </el-card>
...
import {
  ...
  getPublishTasks, createPublishTask, publishNow
} from '@/api/publish'
...
const getStatusType = (status: number): 'success' | 'warning' | 'info' | 'danger' | 'primary' => {
  const map: Record<number, 'success' | 'warning' | 'info' | 'danger' | 'primary'> = {
    0: 'info', 1: 'warning', 2: 'success', 3: 'primary', 4: 'danger',
  }
  return map[status] || 'info'
}

const getStatusText = (status: number) => {
  const map: Record<number, string> = { 0: '草稿', 1: '发布中', 2: '已发布', 3: '定时中', 4: '失败' }
...
const resetTaskForm = () => {
  taskForm.platformAccountId = null
  taskForm.title = ''
  taskForm.content = ''
}
